...
(async () => {
    try {
        ...
        await page.waitForSelector('#internship_list_container_1 .container-fluid.individual_internship.view_detail_button.visibilityTrackerItem', { visible: true });
        console.log('Internship listings loaded.');

        const internships = await page.$$('#internship_list_container_1 .container-fluid.individual_internship.view_detail_button.visibilityTrackerItem');
        console.log(`Found ${internships.length} internships.`);

        for (let i = 0; i < Math.min(10, internships.length); i++) {
            const internship = internships[i];

            // Click on the internship to open the details page
            await internship.click();
            console.log(`Clicked on internship ${i + 1}.`);

            // Wait for potential page updates or detail view
            // await new Promise(resolve => setTimeout(resolve, 5000));
            // console.log('Waited for 5 seconds.');
            // await page.goBack();
            // console.log(`Navigated back after viewing internship ${i + 1}.`);
        }
